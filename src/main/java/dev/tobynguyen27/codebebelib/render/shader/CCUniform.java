...
import dev.tobynguyen27.codebebelib.render.shader.UniformType.Carrier;
    ...
    static CCUniform makeUniform(String name, UniformType type, int count, @Nullable Shader parent) {
        if (count % type.getSize() != 0)
            throw new IllegalArgumentException(
                    "Expected count to be a multiple of the uniform type size: " + type.getSize());
    ...
    private abstract static class UniformEntry<T> extends CCUniform {
        ...
        public void glUniformI(int... values) {
            if (type.getCarrier() != Carrier.INT && type.getCarrier() != Carrier.U_INT) {
                throw new IllegalArgumentException(
                        "Uniform '%s' isn't registered with the carrier of INT or U_INT, Got type '%s' with carrier '%s'."
                                .formatted(getName(), type, type.getCarrier()));
            }
            set((T) values, false);
        }
        ...
        public void glUniformF(boolean transpose, float... values) {
            if (type.getCarrier() != Carrier.FLOAT && type.getCarrier() != Carrier.MATRIX) {
                throw new IllegalArgumentException(
                        "Uniform '%s' isn't registered with the carrier of FLOAT or MATRIX, Got type '%s' with carrier '%s'."
                                .formatted(getName(), type, type.getCarrier()));
            }
        ...
        public void glUniformD(boolean transpose, double... values) {
            if (type.getCarrier() != Carrier.DOUBLE && type.getCarrier() != Carrier.D_MATRIX) {
                throw new IllegalArgumentException(
                        "Uniform '%s' isn't registered with the carrier of DOUBLE or D_MATRIX, Got type '%s' with carrier '%s'."
                                .formatted(getName(), type, type.getCarrier()));
            }
            set((T) values, transpose);
        }
        ...
        public void set(T values, boolean transpose) {
            ...
            if (len(values) != getCount()) {
                throw new IllegalArgumentException("Invalid size for uniform '%s', Expected: '%s', Got: '%s'."
                        .formatted(getName(), getCount(), len(values)));
            }
