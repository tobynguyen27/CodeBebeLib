...
import dev.tobynguyen27.codebebelib.render.shader.GlslProcessor.ProcessedShader;
...
public class CCShaderInstance extends ShaderInstance {
    ...
    protected CCShaderInstance(ResourceProvider resourceProvider, ResourceLocation loc, VertexFormat format)
            throws IOException {
        super(resourceProvider, loc.toString(), format);
    }

    public static CCShaderInstance create(
            ResourceProvider resourceProvider, ResourceLocation loc, VertexFormat format) {
        try {
    ...
    protected void parseUniformNode(JsonElement json) throws ChainedJsonException {
        ...
        if (type == null) {
            throw new ChainedJsonException(
                    "Invalid type '%s'. See UniformType enum. All vanilla types supported.".formatted(typeStr));
        }
        ...
        if (jsonValues.size() != count && jsonValues.size() > 1) {
            throw new ChainedJsonException(
                    "Invalid amount of values specified (expected " + count + ", found " + jsonValues.size() + ")");
        }
    ...
    public Program compileProgram(ResourceProvider resourceProvider, Program.Type programType, ResourceLocation loc)
            throws IOException {
        ResourceLocation adjustedLoc =
                new ResourceLocation(loc.getNamespace(), "shaders/core/" + loc.getPath() + programType.getExtension());
        ...
        if (GL20.glGetShaderi(id, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            ...
            throw new IOException("Couldn't compile " + programType.getName() + " program ("
                    + processedShader.sourceName() + ", " + adjustedLoc + ") : " + s1);
        }
